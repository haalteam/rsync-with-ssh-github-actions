...
run_rsync(){
    sh -c "rsync $INPUT_ARGS -rPz -e 'ssh -i $SSH_PATH/deploy_key --exclude '.git' -o VerifyHostKeyDNS=yes -p $INPUT_PORT' $GITHUB_WORKSPACE/ $INPUT_USER@$INPUT_HOST:$INPUT_DESTINATION"
}
